...
export const RecentItem = ({ doc, alt }) => {
  ...
  return <div className={"px-3 flex cursor-pointer py-2 text-xs bg-gray-100 bg-gradient-to-tr text-white " + (alt ? `  from-purple-400 to-red-500 ` : ` from-blue-400  to-green-500 `)}>
    ...
    <div className="w-14 inline-flex items-center justify-center ">
      <div className="p-3 border border-white" onClick={() => { removeDoc({ doc }) }}>Remove</div>
    </div>
  </div>
}
...
export const OpenFolderPage = () => {
  ...
  return <div className="full flex">
    <div className="h-full w-1/2 border-r border-gray-600">
      <div className="p-3 text-xl bg-gray-600 text-white">
        <div>EffectNode <span className={'text-sm'}>v1.01</span></div>
      </div>
      <div onClick={openFolder} className="px-3 py-2 text-sm cursor-pointer bg-green-200 text-green-800">
        <div>Open Folder</div>
      </div>
