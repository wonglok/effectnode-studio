import { WebGLCubeRenderTarget, Camera, Scene, Mesh, PlaneBufferGeometry, ShaderMaterial, CubeRefractionMapping, BackSide, NoBlending, BoxBufferGeometry, CubeCamera, Color, LinearMipmapLinearFilter } from 'three'
...
export class ShaderCubeDarkness {
  constructor ({ renderer, res = 72, color = new Color('#ffffff') }) {
    // this.onLoop = ctx.onLoop
    // console.log(renderer)
    this.renderer  = renderer
    this.resX = res
    this.renderTargetCube = new CustomWebGLCubeRenderTarget(this.resX, { format: RGBFormat, generateMipmaps: true, magFilter: LinearFilter, minFilter: LinearMipmapLinearFilter })
    this.renderTargetPlane = new WebGLRenderTarget(this.resX, this.resX, { format: RGBFormat, generateMipmaps: true, magFilter: LinearFilter, minFilter: LinearMipmapLinearFilter })
      ...
      fragmentShader: `
        ...
        void main (void) {
          ...
          gl_FragColor = vec4(vec3(
            pattern(uv * 7.0 + -0.23 * cos(time * 0.15)),
            pattern(uv * 7.0 +  0.0 * cos(time * 0.15)),
            pattern(uv * 7.0 +  0.23 * cos(time * 0.15))
          ) * diffuse, 1.0);
          // gl_FragColor = water;
        }
