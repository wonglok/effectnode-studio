...
export const useWorkbench = ({ projectRoot }) => {
  ...
  const db = getLowDB({
    filePath: path.join(projectRoot, "/src/js/meta.json"),
  });
  ...
  useEffect(() => {
    let saveInstant = (json) => {
      ...
      fs.writeFileSync(
        path.join(projectRoot, "./src/js/meta.json"),
...
export const useServer = ({ projectRoot }) => {
  const [refresh, setRefresh] = useState(0);
  const [port, setPort] = useState(3333);

  useEffect(() => {
    let onReady = ({ onDonePack, pack, port }) => {
      setPort(port);
      onDonePack(() => {
        setRefresh((r) => r + 1);
      });
    ...
    let clean = runSession({ projectRoot, onReady });
  ...
  return {
    port,
    previewURL: `http://localhost:${port}/?r=${refresh}`,
  };
};
